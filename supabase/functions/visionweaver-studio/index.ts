...
const RUNWAY_LONG_VIDEO_MODEL = 'seedance2_5';
...
function cors(req) {
  ...
  return {
    'access-control-allow-origin': ALLOWED_ORIGINS.has(origin) || isProjectDeployment
      ? origin
      : 'https://master-ceo-dashboard.vercel.app',
...
async function runway(path, init = {}) {
  ...
  if (!key || !/^key_[0-9a-f]{128}$/.test(key)) {
    throw new Error('Runway access is not configured correctly');
  }
...
async function kling(path, init = {}) {
  ...
  if (!result.ok || (typeof json.code === 'number' && json.code !== 0)) {
    throw new Error(
      'Kling ' + result.status + ': ' +
      String(json.message || json.error || text).slice(0, 400)
    );
  }
  return json.data || json;
}

async function elevenLabsSound(prompt, duration) {
  ...
  if (!result.ok) {
    throw new Error('ElevenLabs ' + result.status + ': ' + (await result.text()).slice(0, 400));
  }
  return result.blob();
}
...
async function providerHealth() {
  ...
  let runwayStatus = null;
  let klingStatus = null;
  ...
  if (klingAccess && klingSecret) {
    try {
      const token = await klingToken();
      const result = await fetch(
        await klingBase() + '/v1/videos/text2video/00000000-0000-0000-0000-000000000000',
      ...
      klingVerified =
        result.status !== 401 &&
        result.status !== 403 &&
        !(typeof body.code === 'number' && [1001, 1002, 1003, 1004].includes(body.code));
    } catch (_) {}
  }

  if (elevenLabsKey) {
    try {
      const result = await fetch(ELEVENLABS_BASE + '/models', {
  ...
  const value = {
    providers: {
      ...
      kling: {
        configured: Boolean(klingAccess && klingSecret),
        verified: klingVerified,
        status: klingStatus
      },
      elevenlabs: {
...
function models() {
  return {
    image: {
      provider: 'automatic',
      model: 'Runway Gen-4 Image Turbo / Kling 2.0 fallback',
      operation: 'text_to_image'
    },
    video: {
      provider: 'automatic',
      model: 'Runway Seedance 2.5 (4–30s) / Kling fallback (5–10s)',
      operation: 'text_to_video'
    },
    audio: {
      provider: 'automatic',
      model: 'ElevenLabs Sound Effects / Runway fallback',
      operation: 'sound_effect'
    },
    book: {
      provider: 'anthropic',
      model: 'claude-sonnet-4-6',
      operation: 'author_package'
    },
    movie: {
      provider: 'automatic',
      model: 'Claude Sonnet 4.6 + verified video provider',
      operation: 'movie_package'
    }
  };
}

async function routeFor(mediaType, parameters = {}) {
  ...
  if (mediaType === 'movie') {
    const videoProvider = health.providers.runway.verified
      ? 'runway'
      : health.providers.kling.verified
        ? 'kling'
        : 'unavailable';
    if (!health.providers.anthropic.verified) {
      throw new Error('No verified planning provider is available');
    }
    return {
      provider: `anthropic+${videoProvider}`,
      model: `claude-sonnet-4-6 + ${videoProvider}`,
      operation: 'movie_package'
    };
  }

  if (mediaType === 'audio') {
    if (health.providers.elevenlabs.verified) {
      return {
        provider: 'elevenlabs',
  ...
  if (mediaType === 'video') {
    const requestedDuration = Math.max(1, Number(parameters.duration) || 5);
    if (health.providers.runway.verified) {
      return {
        provider: 'runway',
        model: RUNWAY_LONG_VIDEO_MODEL,
        operation: 'text_to_video'
      };
    }
    if (health.providers.kling.verified) {
      if (requestedDuration > 10) {
        throw new Error(
          'The requested video is longer than the verified Kling route supports. ' +
          'A verified Runway connection is required for 11–30 second single-shot video.'
        );
      }
      return {
        provider: 'kling',
        model: await setting('kling_video_model', 'kling-v2-6'),
        operation: 'text_to_video'
      };
    }
    throw new Error('No verified video provider is available');
  }
  ...
  if (health.providers.kling.verified) {
    return {
      provider: 'kling',
      model: await setting('kling_image_model', 'kling-v2'),
      operation: 'text_to_image'
    };
  }
...
async function createProject(user, body) {
  ...
  const title = String(
    body.title || (mediaType[0].toUpperCase() + mediaType.slice(1) + ' project')
  ).slice(0, 160);
  const params = body.parameters && typeof body.parameters === 'object'
    ? body.parameters
    : {};
  const route = await routeFor(mediaType, params);
  ...
  const { data: project, error: projectError } = await db.from('vw_projects').insert({
    ...
    settings: params,
    story_object: { source: 'visionweaver-studio', version: 2 }
  }).select('*').single();
  ...
  const { data: fresh } = await db.from('vw_generations')
    .select('*')
    .eq('id', generation.id)
    .single();
...
async function submitTextPackage(generation) {
  ...
  await db.from('vw_projects').update({
    ...
    outputs: {
      ...
      format: isBook ? 'book_package' : 'movie_package'
    },
    error: null
...
async function submitGeneration(generation) {
  if (generation.media_type === 'book' || generation.media_type === 'movie') {
    await submitTextPackage(generation);
    return;
  }
  ...
  if (generation.provider === 'elevenlabs') {
    ...
    const blob = await elevenLabsSound(
      generation.prompt,
      Number(parameters.duration) || 8
    );
    const path =
      generation.owner_id + '/' +
    ...
    await db.from('vw_generations').update({
      status: 'complete',
      storage_paths: [path],
      result: { provider_status: 'SUCCEEDED', output_count: 1 },
      completed_at: new Date().toISOString(),
      error: null
    }).eq('id', generation.id);
    ...
    await db.from('vw_projects').update({
      status: 'complete',
      outputs: { generation_id: generation.id, storage_paths: [path] },
      error: null
    }).eq('id', generation.project_id);
  ...
  if (generation.provider === 'kling') {
    ...
    const payload = isImage
      ? {
          model_name: generation.model || await setting('kling_image_model', 'kling-v2'),
          prompt: generation.prompt.slice(0, 1000),
          aspect_ratio: ratioMap[parameters.ratio] || '16:9',
          n: 1
        }
      : {
          model_name: generation.model || await setting('kling_video_model', 'kling-v2-6'),
          prompt: generation.prompt.slice(0, 1000),
    ...
    const task = await kling(
      isImage ? '/v1/images/generations' : '/v1/videos/text2video',
  ...
  if (generation.media_type === 'image') {
    ...
    path = '/text_to_video';
    const duration = Math.max(4, Math.min(30, Number(parameters.duration) || 5));
    const ratio = ['1280:720', '720:1280', '1920:1080', '1080:1920'].includes(parameters.ratio)
      ? parameters.ratio
      : '1280:720';
    payload = {
      model: RUNWAY_LONG_VIDEO_MODEL,
      promptText: generation.prompt.slice(0, 15000),
      ratio,
      duration,
      audio: false
    };
...
async function mirrorOutputs(generation, urls) {
  const paths = [];
  for (let index = 0; index < urls.length; index += 1) {
    try {
      ...
      if (length > 250 * 1024 * 1024) continue;
...
async function pollGeneration(generation) {
  if (
    !generation.external_id ||
    !['image', 'video', 'audio'].includes(generation.media_type)
  ) return;
  ...
  if (generation.provider === 'kling') {
    ...
    const outputs = generation.media_type === 'image'
      ? task.task_result?.images
      : task.task_result?.videos;
    urls = Array.isArray(outputs)
      ? outputs.map((item) => item.url).filter((item) => typeof item === 'string')
      : [];
  } else {
    task = await runway('/tasks/' + encodeURIComponent(generation.external_id));
    status = String(task.status || '').toUpperCase();
    urls = Array.isArray(task.output)
      ? task.output.filter((item) => typeof item === 'string')
      : [];
  }

  const polls = (generation.poll_count || 0) + 1;
  ...
  if (status === 'SUCCEEDED' || status === 'SUCCEED') {
    ...
    await db.from('vw_generations').update({
      ...
      result: {
        ...
        output_count: urls.length,
        requested_duration_seconds: generation.media_type === 'video'
          ? Math.max(4, Math.min(30, Number(generation.parameters?.duration) || 5))
          : undefined
      },
      poll_count: polls,
      completed_at: new Date().toISOString(),
...
async function tick(ownerId, generationId) {
  let query = db.from('vw_generations')
    .select('*')
...
async function retryGeneration(user, generationId) {
...
async function listWorkspace(user) {
  ...
  const paths = [
    ...new Set(
      (generations || [])
        .flatMap((item) => item.storage_paths || [])
        .concat((assets || []).map((item) => item.storage_path).filter(Boolean))
    )
  ];
  ...
  return {
    ...
    generations: (generations || []).map((item) => ({
      ...item,
      playable_urls: (item.storage_paths || [])
        .map((path) => signed.get(path))
        .filter(Boolean)
        .concat(item.output_urls || [])
    })),
    assets: (assets || []).map((item) => ({
      ...item,
      playable_url: item.storage_path
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('[visionweaver-studio] request', {
      method: req.method,
      path: url.pathname
    });
    ...
    if (req.method === 'GET' && url.pathname.endsWith('/health')) {
      ...
      return response(req, {
        ...
        capabilities: models(),
        limits: {
          video_single_shot_seconds: {
            runway_seedance2_5: { min: 4, max: 30 },
            kling_fallback: { min: 5, max: 10 }
          },
          longform: {
            strategy: 'sequential_scene_extend',
            orchestrator: 'visionweaver-orchestrator'
          }
        },
        ...health
      });
    ...
    if (body.action === 'tick' && await isCron(req)) {
      ...
      console.log('[visionweaver-studio] tick complete', {
        count: acted.length,
        acted
      });
    ...
    if (!user) {
      return response(req, {
        ok: false,
        error: 'production_sign_in_required'
      }, 401);
    }
    ...
    if (body.action === 'retry') {
      return response(req, {
        ok: true,
        ...(await retryGeneration(user, String(body.generation_id || '')))
      });
    }
    ...
    console.error('[visionweaver-studio] request failed', {
      error: String(error)
    });
    return response(req, {
      ok: false,
      error: String(error).replace(/^Error:\s*/, '').slice(0, 1000)
    }, 500);
  }
});
