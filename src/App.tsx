...
const decisions = [
  {title:'VisionWeaver production monitoring', owner:'Technology', due:'Today', level:'Medium'},
...
const pages: Record<string,PageModel> = {
  'Decisions & Approvals':{question:'What requires executive authorization now?',metrics:[['Open decisions','02','1 due today'],['Approved this week','13','100% audited'],['At risk','00','Clear'],['Avg. response','4.2h','Within target']],queue:[['VisionWeaver production monitoring','Technology','Medium'],['LandWeaver provider shortlist','Property Intelligence','Medium'],['GrantOS submission authority','Funding Office','High']],controls:['Approve','Return for evidence','Delegate','Escalate']},
  ...
  'System Portfolio':{question:'Which systems are operational, blocked, degraded, or under construction?',metrics:[['Registered','17','4 reporting'],['Healthy','04','No incidents'],['In build','08','On roadmap'],['Blocked','00','Clear']],queue:[['VisionWeaver','Production','100%'],['CEO Dashboard','Production','100%'],['LandWeaver','Specification ready','35%'],['GrantOS','Reconciliation','44%']],controls:['Open system','Request health check','View dependencies','Open incidents']},
  'Projects & Initiatives':{question:'Are strategic initiatives on plan?',metrics:[['Active','11','9 on plan'],['Milestones','06','This week'],['Blocked','00','Clear'],['Budget health','91%','Within plan']],queue:[['CEO Dashboard production','Technology','Operational'],['VisionWeaver production','Technology','Operational'],['LandWeaver implementation','Property','Queued']],controls:['Inspect','Reprioritize','Request briefing','View roadmap']},
  ...
  'Risk, Security & Compliance':{question:'What can materially harm the enterprise?',metrics:[['Open incidents','00','Clear'],['Security gates','09','Verified'],['Compliance','100%','Current'],['Overdue fixes','00','Clear']],queue:[['VisionWeaver API authentication','Security','Verified'],['OAuth session reset','Technology','Verified'],['Provider license inventory','LandWeaver','Medium']],controls:['Acknowledge','Assign remediation','Escalate','Inspect evidence']},
  'Quality Control':{question:'What has failed or not met Architect standards?',metrics:[['Open findings','08','0 blocking'],['Certified releases','03','Current cycle'],['Regression pass','100%','All checks'],['Quality gain','35%','Above target']],queue:[['CEO Dashboard responsive verification','QC','Verified'],['VisionWeaver tenant isolation','Security QC','Verified'],['LandWeaver provenance display','Product QC','Required']],controls:['Reject promotion','Request remediation','Inspect evidence','Certify']},
...
export function CSuiteDashboard({initialActive='Executive Overview',onNavigate}:{initialActive?:string;onNavigate?:(page:string)=>void}){
  ...
  const act=(label:string)=>setNotice(`${label} is ready. Sign in with the required authority to submit a governed write; this preview will not fail silently.`);
  const portfolio=command.systems.length?command.systems.map(s=>[s.system_name,s.lifecycle_state,`${s.progress_percent}%`]):[['VisionWeaver','Production','100%'],['CEO Dashboard','Production','100%'],['LandWeaver','Specification ready','35%'],['GrantOS','Reconciliation','44%']];
  return <div className="app-shell">
    ...
    <main>
      ...
      <section className="content">{notice&&<div className="data-state loading" role="status"><b>Executive notifications</b><span>{notice}</span><button type="button" onClick={()=>setNotice('')}>Dismiss</button></div>}{active==='Property Intelligence'?<LandWeaverWorkspace/>:active==='Executive Overview'?<>
        <div className="briefing"><div><span className="eyebrow">EXECUTIVE OPERATING BRIEFING</span><h2>Good morning. Two decisions require your attention.</h2><p>VisionWeaver is production-ready and CEO Dashboard identity controls are active. Provider health remains governed at the source.</p></div><button onClick={()=>selectPage('Decisions & Approvals')}>Open daily briefing <ChevronRight/></button></div>
        ...
        <div className="grid">
          ...
          <section className="panel timeline"><div className="panel-title"><div><span className="eyebrow">GOVERNED ACTIVITY</span><h3>Audit pulse</h3></div><Clock3/></div>{['Production access reset activated','Dashboard identity controls verified','VisionWeaver provider access verified'].map((x,i)=><div className="event" key={x}><i/><div><b>{x}</b><small>{i===0?'Today · Executive authority':'Today · System orchestration'}</small></div></div>)}</section>
