...
function ModulePage({module,data,activity,onCreate,onTransition}:{module:Module;data:ModuleRecord[];activity:ModuleActivity[];onCreate:(key:string,input:{name:string;category:string;status_value:string;activity:string})=>Promise<{error:string|null}>;onTransition:(record:ModuleRecord,state:ModuleRecord['record_state'],note:string)=>Promise<{error:string|null}>}){
 const[adding,setAdding]=useState(false),[saving,setSaving]=useState(false),[error,setError]=useState(''),[moving,setMoving]=useState<string|null>(null);
