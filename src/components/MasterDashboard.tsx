...
export default function MasterDashboard({onOpenSuite,onOpenLandWeaver}:{onOpenSuite:()=>void;onOpenLandWeaver:()=>void}){
 ...
 const systems=command.systems.length?command.systems.slice(0,4):[
  {system_name:'VisionWeaver',progress_percent:72,lifecycle_state:'Staging'},{system_name:'CEO Dashboard',progress_percent:100,lifecycle_state:'Active'},{system_name:'LandWeaver',progress_percent:92,lifecycle_state:'MVP'},{system_name:'GrantOS',progress_percent:44,lifecycle_state:'Build'}];
 return <div className="master-shell"><aside className={navOpen?'master-nav open':'master-nav'}>
  ...
  {active==='Dashboard'?<Dashboard systems={systems} onOpenSuite={onOpenSuite} onOpenLandWeaver={onOpenLandWeaver}/>:<ModulePage module={selected} data={command.moduleRecords.filter(r=>r.module_key===active)} onCreate={command.createModuleRecord}/>}</section>
 </main></div>
}

function Dashboard({systems,onOpenSuite,onOpenLandWeaver}:{systems:any[];onOpenSuite:()=>void;onOpenLandWeaver:()=>void}){
