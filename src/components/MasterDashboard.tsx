...
export default function MasterDashboard({onOpenSuite}:{onOpenSuite:()=>void}){
 ...
 return <div className="master-shell"><aside className={navOpen?'master-nav open':'master-nav'}>
  ...
  {active==='Dashboard'?<Dashboard systems={systems} onOpenSuite={onOpenSuite}/>:<ModulePage module={selected} data={command.moduleRecords.filter(r=>r.module_key===active)} onCreate={command.createModuleRecord}/>}</section>
 </main></div>
}
