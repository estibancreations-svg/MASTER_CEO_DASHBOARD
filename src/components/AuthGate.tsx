...
export default function AuthGate({children}:{children:ReactNode}){
 ...
 useEffect(()=>{const client=supabase;if(!client)return;let alive=true;
  const resolve=async(next:Session|null)=>{if(!alive)return;setSession(next);setIdentity(null);setError('');if(!next){setReady(true);return}setReady(false);
   ...
   setIdentity({session:next,user:next.user,organizationId:organization.id,organizationName:organization.display_name,organizationSlug:organization.slug,role:membership.role,scopes:membership.scopes||[],signOut:async()=>{await client.auth.signOut()}});setReady(true)
  };
