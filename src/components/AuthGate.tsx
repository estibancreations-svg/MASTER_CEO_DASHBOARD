...
export default function AuthGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const resolve = async (next: Session | null) => {
      ...
      setIdentity({
        session: next, user: next.user, isBuilder: false,
        organizationId: organization.id, organizationName: organization.display_name, organizationSlug: organization.slug,
        role: membership.role, scopes: membership.scopes || [],
        signOut: async () => { const { error: signOutError } = await client.auth.signOut({ scope: 'global' }); if (signOutError) throw signOutError; }
      });
      setReady(true);
    };

    const bootstrap = async () => {
      if (localStorage.getItem(AUTH_RESET_KEY) !== AUTH_RESET_VERSION) {
        await client.auth.signOut({ scope: 'local' });
        localStorage.setItem(AUTH_RESET_KEY, AUTH_RESET_VERSION);
      }
  ...
  if (!session) return <><SystemControls /><div className="auth-screen"><form className="auth-card" onSubmit={login}>
    ...
    <div className="social-auth" aria-label="Social sign-in options">{SOCIAL_PROVIDERS.map((option) => <button key={option.provider} type="button" onClick={() => void socialLogin(option)} disabled={Boolean(submitting)}><span>{option.mark}</span>{submitting === option.provider ? 'Connecting…' : `Continue with ${option.label}`}</button>)}</div>
